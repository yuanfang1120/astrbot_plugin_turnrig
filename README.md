...
以「监听 `123456789` 群里 `987654321` 的发言，转发到我自己的 QQ」为例：
...
之后让 `987654321` 在那个群里发一条消息，你的私聊就会收到转发。
...
AstrBot 的会话 ID 是 `平台实例ID:消息类型:会话ID`，例如 `qq:GroupMessage:123456789`。
...
- **[DeepSeek](https://www.deepseek.com/)** —— 参与问题排查、代码修复与文档撰写
  （如「群内指定用户」监听因平台前缀不匹配而永不生效、非 QQ 平台转发因消息链类型错误而失败等问题的定位与修复）

所有改动均经维护者在真实环境中实测验证后提交，最终对代码负责的是维护者。
